        ...
        <table class="order-item" v-for="record in records" :key="record.id">
          ...
          <tbody>
            ...
            >
              ...
              >
                <ul class="unstyled">
                  <li>总金额¥{{ orderDetail.totalAmount }}</li>
                  <li>
                    {{
              ...
              >
                <a href="#" class="btn"
                  >{{ record.processStatus === "UNPAID" ? "未完成" : "已完成" }}
                </a>
              </td>
              <td
                v-if="index === 0"
...
export default {
  ...
  methods: {
    async getCenterList() {
      ...
      const result = await this.$API.reqCenterList(page, limit);
      if (result.code === 200) {
  ...
  computed: {
    records() {
      return this.centerList.records || [];
    },
  },
};
</script>
